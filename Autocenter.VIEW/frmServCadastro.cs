...
namespace Autocenter.VIEW
{
    public partial class frmServCadastro : Form
    {
        ...
        private void btnServRelatorio_Click(object sender, EventArgs e)
        {
            ReportDocument oReport = new ReportDocument();
            oReport.Load(Application.StartupPath + "\\CrystalReportServico.rpt");
            oReport.VerifyDatabase();
            oReport.SetDataSource(grvServPesquisa.DataSource as DataTable);
            oReport.Refresh();
            string path = Environment.GetFolderPath(Environment.SpecialFolder.Desktop);
            oReport.ExportToDisk(ExportFormatType.PortableDocFormat, path+"\\Relatório Serviço.pdf");
            MessageBox.Show("Relatório Emitido com sucesso", "OK");
            {
                Process.Start(path + "\\Relatório Serviço.pdf");
            }
        }
    }
}
